...
local ScreenPoint = camera:WorldToScreenPoint(v.Character:WaitForChild("HumanoidRootPart", 5).Position)
...
function handler:getNearPlayers(maxDistance)
    local nearPlayers = {}
...
function handler:removePlayer(plr)
    if not handler.players[plr] then return end
    handler.players[plr] = nil
    handler:removeEntity(plr.Character)
    if handler.connections[plr] then
        for _, connection in next, handler.connections[plr] do
            connection:Disconnect()
        end
        handler.connections[plr] = nil
    end
end
